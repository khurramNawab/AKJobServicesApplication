...
// Strategy:
//   /auth/me 401       → SILENT resolve (expected guest state, not an error)
//   /auth/login 401    → reject normally (let Login.jsx handle UI)
//   /auth/refresh 401  → reject normally (session truly dead)
//   Any other 401      → attempt token refresh → retry → redirect on failure
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // ── CASE 1: /auth/me returns 401 — user is simply not logged in.
    // This is a normal "guest check", NOT an error. Resolve silently.
    if (status === 401 && url.includes('/auth/me')) {
      return Promise.resolve({ data: { success: false, data: null } });
    }

    // ── CASE 2: Login or refresh-token fails — don't retry, let caller handle.
    if (status === 401 && (url.includes('/auth/login') || url.includes('/auth/refresh-token'))) {
      return Promise.reject(error);
    }

    // ── CASE 3: Any other 401 — attempt silent token refresh.
    if (status === 401 && !originalRequest._retry) {
      originalRequest._retry = true;
      try {
        await api.post('/auth/refresh-token');
        return api(originalRequest); // Retry with fresh cookies
      } catch (refreshError) {
        // Refresh failed — session is truly dead, redirect to login
