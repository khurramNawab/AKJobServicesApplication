  ...
  return (
    <div className="min-h-screen bg-[#020617] pt-40 pb-32 px-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto space-y-24 relative z-10 text-left">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="grid grid-cols-2 gap-6">
               {[
                 ...
                 { label: 'High Demand', val: 'Management', icon: Activity, color: 'text-[#2563EB]' }
               ].map((stat, i) => (
              ...
              {filteredData.length > 0 ? (
                filteredData.map((data, i) => (
                  <motion.div
                    ...
                    whileHover={{ y: -10 }}
                    className="glass-card p-10 rounded-[3rem] border-white/5 space-y-8 group hover:border-[#2563EB]/30 transition-all shadow-2xl relative flex flex-col h-full"
                  >
                    <div className="flex justify-between items-start">
                       <div className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:bg-[#2563EB]/10 group-hover:border-[#2563EB]/40 transition-all">
                          <BarChart3 className="w-6 h-6 text-white/40 group-hover:text-[#2563EB] transition-colors" />
                       </div>
                       <div className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest py-1.5 px-3 rounded-full border shadow-inner ${data.growth === 'up' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-[#EF4444]/10 border-[#EF4444]/20 text-[#EF4444]'}`}>
                          {data.growth === 'up' ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />}
                          {data.trend}
                       </div>
                    </div>
                    <div className="flex-1 space-y-2">
                        <h4 className="text-xl font-black text-white tracking-tight group-hover:text-[#2563EB] transition-colors line-clamp-2 leading-snug">{data.role}</h4>
                    ...
                    <Link to="/jobs" className="text-[10px] font-black text-[#2563EB] hover:text-white transition-all uppercase tracking-[0.2em] flex items-center justify-between group/link pt-2 leading-none">
                       View Openings <ArrowRight className="w-4 h-4 group-hover/link:translate-x-1 transition-transform" />
                    </Link>

                    {/* Decorative Hover Glow */}
                    <div className="absolute inset-0 bg-gradient-to-br from-[#2563EB]/0 to-[#2563EB]/0 group-hover:from-[#2563EB]/5 group-hover:to-transparent pointer-events-none transition-all duration-500 rounded-[3rem]" />
                  </motion.div>
                ))
              ) : (
        ...
        <div className="glass-card p-12 rounded-[4rem] border-white/5 flex flex-col md:flex-row items-center gap-12 group hover:border-[#2563EB]/30 transition-all shadow-2xl relative overflow-hidden text-left">
           <div className="absolute top-0 right-0 w-96 h-full bg-[#2563EB]/5 blur-[100px] rounded-full translate-x-1/2 -z-10" />
           <div className="w-24 h-24 rounded-3xl bg-[#2563EB]/10 flex items-center justify-center border border-[#2563EB]/20 flex-shrink-0 group-hover:scale-105 transition-transform shadow-inner">
              <Info className="w-10 h-10 text-[#2563EB]" />
           </div>
