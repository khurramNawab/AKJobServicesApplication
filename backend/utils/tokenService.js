...
export const sendTokenResponse = async (user, statusCode, res, req) => {
    ...
    if (req) {
        ...
        await User.findByIdAndUpdate(user._id, {
            $push: {
                sessions: {
                    $each: [{ ipAddress: ip, deviceInfo: ua.substring(0, 200), lastActive: new Date() }],
    ...
    if (req?.headers?.['x-client-type'] === 'mobile') {
        ...
        if (!isTrustedClient) {
            return res.status(403).json({ success: false, message: 'Invalid mobile client signature' });
        }
...
export const rotateRefreshToken = async (req, res) => {
    try {
        ...
        let decoded;
        try {
            decoded = jwt.verify(incomingToken, process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET + '_refresh');
        } catch {
        ...
        if (user.refreshToken !== incomingHash) {
            ...
            await User.findByIdAndUpdate(user._id, { refreshToken: null, sessions: [] });
            return res.status(401).json({ success: false, message: 'Token reuse detected — all sessions invalidated' });
        }
